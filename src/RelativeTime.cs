...
namespace HallLibrary.Extensions
{
	public static class RelativeTime
	{
		public static string GetRelativeTime (DateTime baseDate, bool includeWeeks = true) {
			...
			if (relative.TakeWhile(v => v.Key != IntervalType.Day).All(v => v.Value == 0) && relative.Any(v => v.Key == IntervalType.Day && Math.Abs(v.Value) == 1)) // exactly one day (no weeks/months/years), time is allowed
